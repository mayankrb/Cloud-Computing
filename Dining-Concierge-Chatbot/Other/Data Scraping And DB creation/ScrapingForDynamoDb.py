...
def addRestaurant(data,  restaurants_table, cuisineType):
    with restaurants_table.batch_writer() as batch:
        for record in data:      
            try:
                new_record = {}
                if str(record['id']) in restaurants_dict.keys():
                    continue
                address = " ".join(record['location']['display_address'])
                restaurants_dict[str(record['id'])] = True
                new_record["insertedAtTimestamp"] = str(datetime.datetime.now())
                new_record["Business ID"] = str(record["id"])
                new_record["Name"] = str(record['name'])
                new_record["Address"] = address
                new_record["Coordinates"] = (Decimal(str(record['coordinates']['latitude'])), Decimal(str(record['coordinates']['longitude'])))
                new_record["Number of Reviews"] = str(record['review_count'])
                new_record["Rating"] = Decimal(str(record["rating"]))
                new_record["Zip Code"] = str(record['location']['zip_code'])    
                new_record["Distance"] = Decimal(str(record['distance']))
                new_record['is_closed'] = str(record["is_closed"])
...
restaurants_table = client.Table('yelp-restaurants')
batch_size = 50
...
for i in range(0, 1000, batch_size):
    ...
    addRestaurant(response_batch["businesses"], restaurants_table, cuisine)
